...
class DashboardTopbar extends ConsumerWidget {
  final String
  title; // Título dinámico que cambia según la pestaña activa (ej: Resumen General)
  final VoidCallback
  onOpenDrawer; // Callback para desplegar el menú hamburguesa lateral en pantallas móviles
  final VoidCallback
  onCreateDatabase; // Callback para abrir el modal de aprovisionamiento de nueva Base de Datos
  final ValueChanged<int>?
  onSelectTab; // Callback opcional para cambiar la pestaña activa desde el Hub de Servicios
  final void Function(String message, {bool success})?
  onMessage; // Receptor de mensajes SnackBar
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isDark =
        themeMode ==
        ThemeMode.dark; // Booleano: true si el tema oscuro está activo

    // 3. Define los colores dinámicos adaptados al tema visual activo
    final surfaceColor = isDark
        ? AppColors.nightSurface
        : AppColors.daySurface; // Fondo de la barra superior
    final borderColor = isDark
        ? AppColors.nightBorder
        : AppColors.dayBorder; // Color de la línea inferior
    final iconColor = isDark
        ? AppColors.nightTextPrimary
        : AppColors.dayTextPrimary; // Color de los iconos interactivos
    final titleColor = isDark
        ? AppColors.nightTextPrimary
        : AppColors.dayTextPrimary; // Color del título principal

    return Container(
      height: 70, // Altura fija estándar de la barra superior
      padding: EdgeInsets.symmetric(
        horizontal: isDesktop ? 28 : 16,
      ), // Relleno lateral adaptativo
      ...
      child: Row(
        children: [
          ...
          if (!isDesktop) ...[
            IconButton(
              onPressed:
                  onOpenDrawer, // Abre el Drawer lateral en dispositivos móviles
            ...
            const RaftLogo(
              small: true,
            ), // Muestra versión pequeña del logo en móvil
          ] else ...[
            Text(
          ...
          // Acción 2: Centro de Notificaciones del Sistema con Badge contador
          Stack(
          ...
          FilledButton.icon(
            onPressed: () {
              showDialog(
                ...
                builder: (context) => ServicesHubDialog(
                  onCreateDatabase:
                      onCreateDatabase, // Pasa el callback para crear bases de datos
                  onSelectTab:
                      onSelectTab, // Pasa el callback para cambiar de pestaña activa
            ...
            label: Text(
              isDesktop ? 'Ecosistema Raft' : 'Servicios',
            ), // Etiqueta responsiva
